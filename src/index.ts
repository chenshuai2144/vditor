...
class Vditor extends VditorMethod {
    ...
    if (!mergedOptions.i18n) {
      if (
        ...
        addScript(`${mergedOptions.cdn}/dist/js/i18n/${mergedOptions.lang}.js`, i18nScriptID)
          ...
          .catch((error) => {
            this.showErrorTip(
              `GET ${mergedOptions.cdn}/dist/js/i18n/${mergedOptions.lang}.js net::ERR_ABORTED 404 (Not Found)`,
            );
          });
      }
    } else {
  ...
  private init(id: HTMLElement, mergedOptions: IOptions) {
    ...
    addScript(
      mergedOptions._lutePath || `${mergedOptions.cdn}/dist/js/lute/lute.min.js`,
      'vditorLuteScript',
    ).then(() => {

...
const nextIsNode = (range: Range) => {
  ...
  if (!nextNode) {
    ...
    nextNode &&
    nextNode.nodeType !== 3 &&
    nextNode.classList.contains('vditor-ir__node') &&
...
const previousIsNode = (range: Range) => {
  ...
  if (
    ...
    // *em*|text
    previousNode.classList.contains('vditor-ir__node') &&
    !previousNode.getAttribute('data-block')
  ) {
    return previousNode;
  }
  return false;
...
export const expandMarker = (range: Range, vditor: IVditor) => {
  ...
  const nodeElementEnd =
    !range.collapsed && hasTopClosestByClassName(range.endContainer, 'vditor-ir__node');

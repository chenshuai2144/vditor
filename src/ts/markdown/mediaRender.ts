...
const iframeRender = (element: HTMLElement, url: string) => {
  ...
  if (youtubeMatch && youtubeMatch[1].length === 11) {
    element.insertAdjacentHTML(
      ...
      `<iframe class="iframe__video" src="//www.youtube.com/embed/${
        youtubeMatch[1] + (youtubeMatch[2] ? '?start=' + youtubeMatch[2] : '')
      }"></iframe>`,
    );
    element.remove();
 ...
 src="https://www.dailymotion.com/embed/video/${dailymotionMatch[2]}"></iframe>`,
    ...
    (url.indexOf('bvid=') > -1 || (bilibiliMatch && bilibiliMatch[1]))
  ) {
    ...
    element.insertAdjacentHTML('afterend', `<iframe class="iframe__video" src="${src}"></iframe>`);
    element.remove();

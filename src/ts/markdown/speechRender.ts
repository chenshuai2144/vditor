...
export const speechRender = (element: HTMLElement, lang: keyof II18n = 'zh_CN') => {
  ...
  element.addEventListener(window.ontouchstart !== undefined ? 'touchend' : 'click', (event) => {
    ...
    if (
      target.classList.contains('vditor-speech') ||
      target.parentElement.classList.contains('vditor-speech')
    ) {
    ...
    speechDom.style.top =
      rect.top + rect.height + document.querySelector('html').scrollTop - 20 + 'px';
    if (window.ontouchstart !== undefined) {
      speechDom.style.left =
        (event as TouchEvent).changedTouches[(event as TouchEvent).changedTouches.length - 1]
          .pageX +
        2 +
        'px';
    } else {

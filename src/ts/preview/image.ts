export const previewImage = (
  ...
  document.body.insertAdjacentHTML(
    'beforeend',
    `<div class="vditor vditor-img${theme === 'dark' ? ' vditor--dark' : ''}">
    ...
    <div class="vditor-img__img" onclick="this.parentElement.outerHTML = '';document.body.style.overflow = ''">
      <img style="width: ${oldImgElement.width}px;height:${
      oldImgElement.height
    }px;transform: translate3d(${oldImgRect.left}px, ${
      oldImgRect.top - height
    }px, 0)" src="${oldImgElement.getAttribute('src')}">
    </div>
</div>`,
  );
  ...
  const translate3d = `translate3d(${
    Math.max(0, window.innerWidth - oldImgElement.naturalWidth) / 2
  }px, ${Math.max(0, window.innerHeight - height - oldImgElement.naturalHeight) / 2}px, 0)`;
  setTimeout(() => {
    imgElement.setAttribute(
      'style',
      `transition: transform .3s ease-in-out;transform: ${translate3d}`,
    );
    setTimeout(() => {
  ...
  btnElement.addEventListener('click', () => {
    ...
    if (
      (deg / 90) % 2 === 1 &&
      oldImgElement.naturalWidth > imgElement.parentElement.clientHeight
    ) {
      imgElement.style.transform = `translate3d(${
        Math.max(0, window.innerWidth - oldImgElement.naturalWidth) / 2
      }px, ${
        oldImgElement.naturalWidth / 2 - oldImgElement.naturalHeight / 2
      }px, 0) rotateZ(${deg}deg)`;
    } else {

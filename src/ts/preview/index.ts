  ...
  constructor(vditor: IVditor) {
    ...
    for (let i = 0; i < actions.length; i++) {
      ...
      if (typeof action === 'object') {
        actionHtml.push(
          `<button type="button" data-type="${action.key}" class="${action.className}"${
            action.tooltip ? ` aria-label="${action.tooltip}"` : ''
          }">${action.text}</button>`,
        );
        continue;
      }
    ...
    actionElement.addEventListener(getEventName(), (event) => {
      ...
      const actionCustom = actions.find(
        (w: IPreviewActionCustom) => w?.key === type,
      ) as IPreviewActionCustom;
      if (actionCustom) {
  ...
  private copyToX(vditor: IVditor, copyElement: HTMLElement, type = 'mp-wechat') {
    ...
    vditor.tip.show(
      ['zhihu', 'mp-wechat'].includes(type)
        ? `已复制，可到${type === 'zhihu' ? '知乎' : '微信公众号平台'}进行粘贴`
        : `已复制到剪切板`,
    );
  }
}

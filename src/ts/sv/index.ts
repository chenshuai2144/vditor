...
class Editor {
  ...
  private bindEvent(vditor: IVditor) {
    ...
    this.element.addEventListener('scroll', () => {
      ...
      const textScrollHeight =
        this.element.scrollHeight - parseFloat(this.element.style.paddingBottom || '0');
    ...
    this.element.addEventListener('keyup', (event) => {
      ...
      if (
        ...
        (vditor.sv.element.firstElementChild.textContent === '' ||
          vditor.sv.element.textContent === '\n')
      ) {
        // 为空时显示 placeholder
        vditor.sv.element.innerHTML = '';
        return;

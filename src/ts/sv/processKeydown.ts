...
export const processKeydown = (vditor: IVditor, event: KeyboardEvent) => {
  ...
  // blockquote
  let blockquoteMarkerElement = hasClosestByAttribute(
    startContainer,
    'data-type',
    'blockquote-marker',
  );
  if (
    !blockquoteMarkerElement &&
    range.startOffset === 0 &&
    textElement &&
    textElement.previousElementSibling &&
  ...
  // list item
  const listMarkerElement = hasClosestByAttribute(
    startContainer,
    'data-type',
    'li-marker',
  ) as HTMLElement;
  const taskMarkerElement = hasClosestByAttribute(
    startContainer,
    'data-type',
    'task-marker',
  ) as HTMLElement;
  ...
  if (!listLastMarkerElement) {
    if (
      taskMarkerElement &&
      taskMarkerElement.nextElementSibling.getAttribute('data-type') !== 'task-marker'
    ) {
      listLastMarkerElement = taskMarkerElement;
    }
  }
  if (
    !listLastMarkerElement &&
    range.startOffset === 0 &&
    textElement &&
    textElement.previousElementSibling &&
  ...
  if (listLastMarkerElement) {
    ...
    if (startIndex === listLastMarkerElement.textContent.length) {
      ...
      if (
        ...
        if (
          listFirstMarkerElement.previousElementSibling?.getAttribute('data-type') === 'padding'
        ) {
      ...
      if (event.key === 'Tab') {
        if (event.shiftKey) {
          ...
          listFirstMarkerElement.insertAdjacentHTML(
            'beforebegin',
            `<span data-type="padding">${listFirstMarkerElement.textContent.replace(
              /\S/g,
              ' ',
            )}</span>`,
          );
        ...
        if (/^\d/.test(listFirstMarkerElement.textContent)) {
          listFirstMarkerElement.textContent = listFirstMarkerElement.textContent.replace(
            /^\d{1,}/,
            '1',
          );
  ...
  if (event.key === 'Enter' && !isCtrl(event) && !event.altKey && !event.shiftKey && blockElement) {
    ...
    if (
      getSelectPosition(blockElement, vditor.sv.element).start <=
      (newLineMatch ? newLineMatch[0].length : 0)
    ) {
      // 允许段落开始换行
      isFirst = true;
    }
  ...
  if (event.key === 'Backspace' && !isCtrl(event) && !event.altKey && !event.shiftKey) {
    if (
      spanElement &&
      spanElement.previousElementSibling?.getAttribute('data-type') === 'newline' &&
    ...
    if (
      blockElement &&
      getSelectPosition(blockElement, vditor.sv.element, range).start === 0 &&
      ...
      if (
        ...
        !blockElement.previousElementSibling?.querySelector('[data-type="code-block-open-marker"]')
      ) {
        inputEvent(vditor);
      } else {
        if (previousLastElement.getAttribute('data-type') !== 'newline') {

...
export const fixCJKPosition = (range: Range, vditor: IVditor, event: KeyboardEvent) => {
  ...
  if (
    pLiElement &&
    getSelectPosition(pLiElement, vditor[vditor.currentMode].element, range).start === 0
  ) {
...
export const fixCursorDownInlineMath = (range: Range, key: string) => {
  if (key === 'ArrowDown' || key === 'ArrowUp') {
    const inlineElement =
      hasClosestByAttribute(range.startContainer, 'data-type', 'math-inline') ||
...
export const isLastCell = (cellElement: HTMLElement) => {
  ...
  if (
    tableElement &&
    tableElement.lastElementChild.lastElementChild.lastElementChild.isSameNode(cellElement)
  ) {
    return tableElement;
  }
  return false;
...
const goPreviousCell = (cellElement: HTMLElement, range: Range, isSelected = true) => {
  ...
  if (!previousElement) {
    if (cellElement.parentElement.previousElementSibling) {
      ...
      previousElement =
        cellElement.parentElement.parentElement.previousElementSibling.lastElementChild
          .lastElementChild;
    } else {
...
export const insertAfterBlock = (
  ...
  if (
    ...
    if (
      ...
      blockElement.insertAdjacentHTML('afterend', `<p data-block="0">${Constants.ZWSP}<wbr></p>`);
      setRangeByWbr(vditor[vditor.currentMode].element, range);
    } else {
...
export const insertBeforeBlock = (
  ...
  if (
    ...
    if (
      !previousElement ||
      (previousElement &&
        (previousElement.tagName === 'TABLE' || previousElement.getAttribute('data-type')))
    ) {
...
export const listToggle = (vditor: IVditor, range: Range, type: string, cancel = true) => {
  ...
  if (cancel && itemElement) {
    ...
    for (let i = 0; i < itemElement.parentElement.childElementCount; i++) {
      ...
      pHTML += `<p data-block="0">${itemElement.parentElement.children[
        i
      ].innerHTML.trimLeft()}</p>`;
    }
...
export const listIndent = (vditor: IVditor, liElement: HTMLElement, range: Range) => {
  ...
  if (liElement && previousElement) {
    ...
    liElements.forEach((item: HTMLElement) => {
      ...
      liHTML += `<li data-node-id="${item.getAttribute('data-node-id')}" data-marker="${marker}">${
        item.innerHTML
      }</li>`;
      item.remove();
    });
...
export const insertRowAbove = (vditor: IVditor, range: Range, cellElement: HTMLElement) => {
  ...
  for (let m = 0; m < cellElement.parentElement.childElementCount; m++) {
    if (cellElement.tagName === 'TH') {
      rowHTML += `<th align="${cellElement.parentElement.children[m].getAttribute(
        'align',
      )}"> </th>`;
    } else {
      rowHTML += `<td align="${cellElement.parentElement.children[m].getAttribute(
        'align',
      )}"> </td>`;
    }
  }
  ...
  if (cellElement.tagName === 'TD') {
    ...
    if (cellElement.parentElement.previousElementSibling) {
      ...
      range.selectNodeContents(
        tbodyElement.previousElementSibling.lastElementChild.lastElementChild,
      );
    }
  ...
  if (cellElement) {
    ...
    if (!isCtrl(event) && !event.altKey && event.key === 'Enter') {
      if (
        ...
        (cellElement.lastElementChild &&
          (!cellElement.lastElementChild.isSameNode(cellElement.lastChild) ||
    ...
    if (event.key === 'Tab') {
      ...
      if (!nextElement) {
        if (cellElement.parentElement.nextElementSibling) {
          ...
          nextElement =
            cellElement.parentElement.parentElement.nextElementSibling.firstElementChild
              .firstElementChild;
        } else {
          nextElement = null;
        }
    ...
    if (matchHotKey('⇧⌘C', event)) {
      if (vditor.currentMode === 'ir') {
        ...
        const itemElement: HTMLElement =
          vditor.wysiwyg.popover.querySelector('[data-type="center"]');
        if (itemElement) {
          itemElement.click();
          event.preventDefault();
          return true;
...
export const fixTask = (vditor: IVditor, range: Range, event: KeyboardEvent) => {
 ...
 data-tight="true"${dataMarker} data-block="0"${startAttribute}>${afterHTML}</${parentTagName}>`;
        ...
        taskItemElement.insertAdjacentHTML(
          'afterend',
          `<li class="vditor-task" data-marker="${taskItemElement.getAttribute(
            'data-marker',
...
export const fixFirefoxArrowUpTable = (
  ...
  if (
    ...
    range.selectNodeContents(
      (blockElement.nextElementSibling as HTMLTableElement).rows[0].cells[0],
    );
    range.collapse(true);
    event.preventDefault();
    return true;
        ...
        if (xhr.readyState === XMLHttpRequest.DONE) {
          if (xhr.status === 200) {
            ...
            if (vditor.currentMode === 'sv') {
              ...
              const imgElement: HTMLImageElement = vditor[vditor.currentMode].element.querySelector(
                `img[src="${original}"]`,
              );

...
export const highlightToolbarWYSIWYG = (vditor: IVditor) => {
  ...
  vditor.wysiwyg.hlToolbarTimeoutId = window.setTimeout(() => {
    ...
    if (commentElement) {
      ...
      if (
        ids.length > 1 &&
        commentElement.nextSibling.isSameNode(commentElement.nextElementSibling)
      ) {
    ...
    if (tableElement) {
          ...
          for (let i = 0; i < tableElement.rows.length; i++) {
            if (columnDiff > 0) {
              for (let j = 0; j < columnDiff; j++) {
                if (i === 0) {
                  tableElement.rows[i].lastElementChild.insertAdjacentHTML(
                    'afterend',
                    '<th> </th>',
                  );
                } else {
      ...
      center.setAttribute(
        'aria-label',
        window.VditorI18n.alignCenter + '<' + updateHotkeyTip('⇧⌘C') + '>',
      );
      ...
      right.setAttribute(
        'aria-label',
        window.VditorI18n.alignRight + '<' + updateHotkeyTip('⇧⌘R') + '>',
      );
      ...
      insertRowElement.setAttribute(
        'aria-label',
        window.VditorI18n.insertRowBelow + '<' + updateHotkeyTip('⌘=') + '>',
      );
      ...
      insertColumnElement.setAttribute(
        'aria-label',
        window.VditorI18n.insertColumnRight + '<' + updateHotkeyTip('⇧⌘=') + '>',
      );
      ...
      insertColumnBElement.setAttribute(
        'aria-label',
        window.VditorI18n.insertColumnLeft + '<' + updateHotkeyTip('⇧⌘G') + '>',
      );
      ...
      deleteRowElement.setAttribute(
        'aria-label',
        window.VditorI18n['delete-row'] + '<' + updateHotkeyTip('⌘-') + '>',
      );
      ...
      deleteColumnElement.setAttribute(
        'aria-label',
        window.VditorI18n['delete-column'] + '<' + updateHotkeyTip('⇧⌘-') + '>',
      );
    ...
    vditor.wysiwyg.element.querySelectorAll('.vditor-wysiwyg__preview').forEach((itemElement) => {
      if (
        !blockRenderElement ||
        (blockRenderElement && isBlock && !blockRenderElement.contains(itemElement))
      ) {
...
const setPopoverPosition = (vditor: IVditor, element: HTMLElement) => {
  ...
  vditor.wysiwyg.popover.style.left =
    Math.min(
      ...
      vditor.wysiwyg.element.clientWidth - vditor.wysiwyg.popover.clientWidth,
    ) + 'px';
...
const genUp = (range: Range, element: HTMLElement, vditor: IVditor) => {
  ...
  if (
    !previousElement ||
    (!element.parentElement.isEqualNode(vditor.wysiwyg.element) && element.tagName !== 'LI')
  ) {
    return;
  }

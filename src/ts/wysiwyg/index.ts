...
<div class="vditor-panel vditor-panel--none">
      ...
      this.selectPopover.querySelector('button').onclick = () => {
        ...
        if (startElement) {
          if (blockStartElement) {
            ...
            startElement.textContent.trim().replace(Constants.ZWSP, '') === '' &&
            removeStart
          ) {
            startElement.remove();
          }
        ...
        if (endElement) {
          if (blockEndElement) {
            ...
            endElement.textContent.trim().replace(Constants.ZWSP, '') === '' &&
            removeEnd
          ) {
            endElement.remove();
          }
    ...
    if (vditor.currentMode === 'wysiwyg' && vditor.options.comment.enable) {
      ...
      if (getData) {
        this.commentIds.forEach((id) => {
          comments.push({
            id,
            top: (this.element.querySelector(`.vditor-comment[data-cmtids="${id}"]`) as HTMLElement)
              .offsetTop,
          });
        });
        return comments;
  ...
  private bindEvent(vditor: IVditor) {
          ...
          if (
            ...
            const popoverTop =
              Math.max(
                ...
                Math.min(top - vditor.wysiwyg.element.scrollTop, this.element.clientHeight - 21),
              ) + 'px';
    ...
    this.element.addEventListener('scroll', () => {
      ...
      const top =
        parseInt(this.popover.getAttribute('data-top'), 10) - vditor.wysiwyg.element.scrollTop;
      let max = -8;
      if (
        vditor.options.toolbarConfig.pin &&
        vditor.toolbar.element.getBoundingClientRect().top === 0
      ...
      paste(vditor, event, {
        pasteCode: (code: string) => {
          ...
          if (blockElement) {
            ...
            vditor.wysiwyg.element.innerHTML = vditor.lute.SpinVditorDOM(
    ...
    this.element.addEventListener('input', (event: InputEvent) => {
      ...
      if (
        (startSpace && blockElement.getAttribute('data-type') !== 'code-block') ||
        endSpace ||

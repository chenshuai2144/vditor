...
export const previoueIsEmptyA = (node: Node) => {
  ...
  while (previousNode) {
    if (
      ...
      previousNode.innerHTML.replace(Constants.ZWSP, '') === '' &&
...
export const splitElement = (range: Range) => {
  ...
  if (
    (text.substr(0, offset) !== '' && text.substr(0, offset) !== Constants.ZWSP) ||
    previousHTML
  ) {
...
export const modifyPre = (vditor: IVditor, range: Range) => {
  ...
  Array.from(vditor.wysiwyg.element.childNodes).find((node: HTMLElement) => {
    if (node.nodeType === 3) {
      ...
      const cloneRangeOffset =
        range.startContainer.nodeType === 3 ? range.startOffset : node.textContent.length;
      node.parentNode.insertBefore(pElement, node);
      node.remove();
      range.setStart(
        pElement.firstChild,
        Math.min(pElement.firstChild.textContent.length, cloneRangeOffset),
      );
      range.collapse(true);

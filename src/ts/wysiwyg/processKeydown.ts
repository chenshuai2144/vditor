...
export const processKeydown = (vditor: IVditor, event: KeyboardEvent) => {
  ...
  if (codeRenderElement) {
    ...
    if (
      ...
      const inputElemment = vditor.wysiwyg.popover.querySelector(
        '.vditor-input',
      ) as HTMLInputElement;
      inputElemment.focus();
    ...
    if (codeRenderElement.getAttribute('data-block') === '0') {
      ...
      if (
        insertAfterBlock(
          ...
          event,
          range,
          codeRenderElement.firstElementChild as HTMLElement,
          codeRenderElement,
        )
      ) {
        return true;
      ...
      if (
        codeRenderElement.getAttribute('data-type') !== 'yaml-front-matter' &&
        insertBeforeBlock(
          vditor,
          event,
          range,
  ...
  if (headingElement) {
    if (
      headingElement.tagName === 'H6' &&
      startContainer.textContent.length === range.startOffset &&
      !isCtrl(event) &&
      !event.shiftKey &&
      !event.altKey &&
      event.key === 'Enter'
  ...
  if (
    ...
    if (blockElement) {
      if (
        ...
        if (
          (rangeStart === 0 && range.startOffset === 0) || // https://github.com/Vanessa219/vditor/issues/894
          (rangeStart === 1 && blockElement.innerText.startsWith(Constants.ZWSP))
        ) {
          // 当前块删除后光标落于代码渲染块上，当前块会被删除，因此需要阻止事件，不能和 keyup 中的代码块处理合并
          showCode(
            blockElement.previousElementSibling.lastElementChild as HTMLElement,
            vditor,
            false,
          );
      ...
      if (
        ...
        startContainer.textContent =
          startContainer.textContent.substring(0, rangeStartOffset - 1) + Constants.ZWSP;
      ...
      blockElement
        .querySelectorAll("span.vditor-wysiwyg__block[data-type='math-inline']")
        .forEach((item) => {
          (item.firstElementChild as HTMLElement).style.display = 'inline';
          (item.lastElementChild as HTMLElement).style.display = 'none';
        });
      blockElement
        .querySelectorAll("span.vditor-wysiwyg__block[data-type='html-entity']")
        .forEach((item) => {
          (item.firstElementChild as HTMLElement).style.display = 'inline';
          (item.lastElementChild as HTMLElement).style.display = 'none';
        });
  ...
  if (
    isFirefox() &&
    range.startOffset === 1 &&
    startContainer.textContent.indexOf(Constants.ZWSP) > -1 &&
    startContainer.previousSibling &&
  ...
  if (event.key === 'ArrowDown') {
    ...
    if (
      ...
      nextElement.getAttribute('data-type') === 'math-inline'
    ) {

...
const cancelBES = (range: Range, vditor: IVditor, commandName: string) => {
  ...
  while (element && !jump) {
    ...
    if (tagName === 'S' || tagName === 'STRONG' || tagName === 'EM') {
      ...
      if (
        (commandName === 'bold' && tagName === 'STRONG') ||
...
export const toolbarEvent = (vditor: IVditor, actionBtn: Element, event: Event) => {
  if (
    vditor.wysiwyg.composingLock && // Mac Chrome 中韩文结束会出发此事件，导致重复末尾字符 https://github.com/Vanessa219/vditor/issues/188
  ...
  if (actionBtn.classList.contains('vditor-menu--current')) {
    ...
    if (commandName === 'quote') {
      ...
      if (inlineCodeElement) {
        inlineCodeElement.outerHTML =
          inlineCodeElement.innerHTML.replace(Constants.ZWSP, '') + '<wbr>';
        setRangeByWbr(vditor.wysiwyg.element, range);
      }
    ...
    if (commandName === 'quote') {
      ...
      if (range.toString().trim() === '') {
        ...
        const delimiter =
          tableText[0].split(',').length > tableText[0].split('\t').length ? ',' : '\t';
      ...
      if (
        range.toString() === '' &&
        (commandName === 'bold' || commandName === 'italic' || commandName === 'strikeThrough')
      ) {
        let tagName = 'strong';
